export default class RegExLexer {
  // \ ^ $ . | ? * + ( ) [ {
  static CHAR_TYPES = {
    '.': 'dot',
    '|': 'alt',
  ...
  parse(str, parseType) {
    ...
    const closeIndex = parseType === 'all' ? str.lastIndexOf('/') : Infinity;
  ...
  parseChar = (str, token, charset, allowSlash) => {
    const c = str[token.i];
  ...
  parseEsc(str, token, charset, capgroups, closeIndex) {
    // jsMode tries to read escape chars as a JS string which is less permissive than JS RegExp,
    // and doesn't support \c or backreferences, used for subst

    // Note: \8 & \9 are treated differently: IE & Chrome match "8", Safari & FF match "\8",
    // we support the former case since Chrome & IE are dominant
    ...
    /* eslint-disable no-cond-assign, consistent-return */
    if (
      !jsMode &&
      !charset &&
      (match = sub.match(/^\d\d?/)) &&
      (o = capgroups[parseInt(match[0], 10) - 1])
    ) {
    ...
    if ((match = sub.match(/^u[\da-fA-F]{4}/))) {
      ...
      // control char without a code - strangely,
      // this is decomposed into literals equivalent to "\\c"
    ...
    token.clss = 'esc';
    return token;
    /* eslint-enable no-cond-assign, consistent-return */
  }
  ...
  parseQuant = (str, token) => {
    ...
    if (arr[1] == null) {
      ...
      token.max = arr[1] == '' ? -1 : parseInt(arr[1], 10);
    }
